...
public class PurchasingReturnOrderPasser extends PurchasingSalesPasserTemplate<PurchasingOrderVO> {
	private ItemFactory factory=new StorageInReturnItemDaoFactoryImpl();
	private static PurchasingReturnOrderPasser INSTANCE=new PurchasingReturnOrderPasser();
	private String sql="update purchasingreturnorder set status=?,passdate=? where id=?";
	
	public PurchasingReturnOrderPasser(){
	...
	protected boolean updateStorage(Set<PurchasingOrderVO> orders) {
		for(PurchasingOrderVO order:orders){
			...
			for(CommodityItemVO cl:storageInList){
				if(!commodityDao.reduceAmountById(cl.getCommID(), cl.getAmount()))return false;
			}
		}
		return true;
	}
